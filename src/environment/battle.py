...
class BattleState(BaseModel):
    ...
    # 新增：跟踪发言状态
    speech_status: Dict[str, bool] = Field(default_factory=dict)  # 记录agent是否已完成发言

    def mark_spoken(self, agent_id: str) -> None:
        """标记agent已完成发言"""
        self.speech_status[agent_id] = True

    def has_spoken(self, agent_id: str) -> bool:
        """检查agent是否已发言"""
        return self.speech_status.get(agent_id, False)
    ...
    def _recalculate_vote_results(self) -> None:
        ...
        logger.info(
            f"📋 Terminated agents: {list(self.terminated_agents.keys())} (total: {len(self.terminated_agents)})")
...
class BattleEnvironment(BaseEnvironment):
    ...
    async def handle_speak(self, agent_id: str, content: str) -> ToolResult:
        ...
        debate_entry = {
            ...
            "agent_id": agent_id,
            "via_tool": True  # 新增：标记为通过工具调用
        }
        self.state.debate_history.append(debate_entry)

        # 标记发言完成
        self.state.mark_spoken(agent_id)
    ...
    async def _send_initial_context(self, report: Dict[str, Any]) -> None:
        ...
        context_parts.append(
            "请基于以上所有专家的分析结果，进行深度讨论并最终投票决定该股票是看涨(bullish)还是看跌(bearish)。")
    ...
    async def _send_debate_instruction(self, current_agent_id: str, speaker_index: int, round_num: int) -> None:
        ...
        for entry in self.state.debate_history:
            if entry.get("round") == self.state.current_round:  # 只显示当前轮次发言
                speaker_name = entry.get("speaker", "Unknown")
                content = entry.get("content", "")
                if content:
                    prefix = "🗣️ " if entry.get("via_tool") else "💬 "
                    previous_speeches.append(f"{prefix}**{speaker_name}**: {content[:200]}...")
        ...
        if current_agent_id in self.agents:
            ...
            if isinstance(agent, ToolCallAgent):
                ...
                self.llm_calls += 1
                logger.info(
                    f"✉️ Sent debate instruction to {current_agent_id} (Round {round_num}, Speaker #{speaker_index + 1})")
    ...
    async def _run_single_agent_debate_turn(self, agent_id: str) -> None:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                result = await agent.run(f"现在是你的发言时间，请立即表达观点！")

                # 捕获发言内容
                if isinstance(result, str):
                    speech_content = result
                    logger.info(f"💬 Captured speech content from {agent_id}: {speech_content[:50]}...")
                elif hasattr(result, "content") and isinstance(result.content, str):
                    speech_content = result.content
                    logger.info(f"💬 Captured speech content from {agent_id}: {speech_content[:50]}...")

                # 记录发言到历史
                if speech_content and len(speech_content.strip()) >= 30:  # 确保有足够内容
                    agent_name = self.state.active_agents.get(agent_id, agent_id)
                    debate_entry = {
                        "speaker": agent_name,
                        ...
                        "agent_id": agent_id,
                        "via_tool": False  # 标记为直接输出
                    }
                    self.state.debate_history.append(debate_entry)
                    self.state.add_highlight(agent_name, speech_content)
                    self.state.mark_spoken(agent_id)  # 标记发言完成
                    logger.info(f"✅ Auto-recorded speech from {agent_id} (Round {self.state.current_round})")
                else:
                    logger.warning(f"⚠️ {agent_id} submitted invalid speech (too short)")
                    self.state.add_event("invalid_speech", agent_id, content=speech_content)
                ...
                if attempt == max_retries:
                    ...
                    # 创建错误发言记录
                    error_entry = {
                        "speaker": self.state.active_agents.get(agent_id, agent_id),
                        "content": f"发言失败: {str(e)}",
                        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        "round": self.state.current_round,
                        "agent_id": agent_id,
                        "error": True
                    }
                    self.state.debate_history.append(error_entry)
                    self.state.mark_spoken(agent_id)  # 标记发言完成
    ...
    async def _run_final_voting(self) -> None:
        ...
        for agent_id in self.state.active_agents.keys():
            if self.state.can_agent_vote(agent_id):
                ...
                logger.warning(
                    f"⚠️ {agent_id} cannot vote: {'terminated' if agent_id in self.state.terminated_agents else 'unknown reason'}")
...
⚠️ 不要再分析，直接投票！
    ...
    def _validate_final_voting(self) -> None:
        ...
        if missing_voters:
            ...
            logger.error(
                f"❌ This explains why we have {len(self.state.final_votes)} votes instead of {len(expected_voters)}")
